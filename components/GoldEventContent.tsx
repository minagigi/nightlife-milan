  ...
  return (
    <div className="mt-4">
      ...
      <div className="not-prose flex flex-col sm:flex-row gap-3 mb-10">
        ...
        >
          🎟️ {tr(locale, 'Buy Tickets', 'Compra Biglietti')}
        </a>
        <a
          href={affiliateUrl}
          target="_blank"
        ...
        >
          🍾 {tr(locale, 'Book a Table', 'Prenota Tavolo')}
        </a>
      </div>

      {/* Sezioni gold — bilingui, sceglie il campo in base a locale. Fallback
          all'inglese per i blob pubblicati prima dell'aggiunta dei campi IT
          (mai testo vuoto). */}
      {rewritten.sections?.map((s, i) => (
        <div key={i} className="mb-8">
          <h2 className="text-2xl font-serif font-bold text-champagne mb-3">
            {s.emoji} {tr(locale, s.title, s.titleIt)}
          </h2>
          <p className="text-white/70 leading-relaxed">{tr(locale, s.body, s.bodyIt)}</p>
        </div>
      ))}
      ...
      {rewritten.programme?.length > 0 && (
        <div className="mb-12">
          <h2 className="text-2xl font-serif font-bold text-champagne mb-4">
            🗓️ {tr(locale, 'Evening Programme', 'Programma della Serata')}
          </h2>
          <div className="space-y-2">
            {rewritten.programme.map((slot, i) => (
              <div key={i} className="flex gap-4 text-sm border-b border-white/8 py-2">
                ...
                <span className="text-white/60">{tr(locale, slot.title, slot.titleIt)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {(tickets.length > 0 || tables.length > 0 || guestlists.length > 0) && (
        <div className="mb-12">
          <h2 className="text-2xl font-serif font-bold text-champagne mb-4">
            🎟️ {tr(locale, 'Tickets & VIP Tables', 'Prezzi & Tavoli VIP')}
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 not-prose">
            {[...tickets, ...guestlists, ...tables].map((o, i) => (
              <div key={i} className="p-4 rounded-xl border border-white/8 bg-white/[0.02] flex justify-between items-center">
                ...
                <span className="text-champagne font-bold">{o.price === 0 ? tr(locale, 'Free', 'Gratis') : `€${o.price}`}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-3">
            {rewritten.faqLong.map((faq, index) => (
              <div key={index} className="bg-white/[0.03] rounded-xl border border-white/10">
                ...
                >
                  <span className="text-white font-medium text-sm">{tr(locale, faq.question, faq.questionIt)}</span>
                  <ChevronDown className={`text-champagne shrink-0 transition-transform ${openIndex === index ? 'rotate-180' : ''}`} size={18} />
                </button>
                {openIndex === index && (
                  <div className="px-5 pb-5 text-white/50 text-sm leading-relaxed">{tr(locale, faq.answer, faq.answerIt)}</div>
                )}
              </div>
            ))}
          </div>
