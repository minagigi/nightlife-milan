...
✓ .Codex/QUICK_START.md          # Commands & dev server
✓ .Codex/ARCHITECTURE_MAP.md     # File locations
```

**At task completion:**
- Save key decisions in `.Codex/completions/YYYY-MM-DD-task.md`
- When the user approves a new publishing pattern, update both the global and
  `.agents/skills/nightlife-milan-publishing` copies, plus an enforceable
  repository rule or test, before declaring the task complete.

**⚠️ NEVER auto-load:**
- `.Codex/completions/` · `.Codex/sessions/` · `docs/archive/`
...
- **Routing**: `app/[locale]/` — ~35 enabled locales defined in
  `lib/i18n/locales.ts` (source of truth; `en` default, no prefix). Enabled and
  indexed are separate states. Only locales with complete native content may
  enter robots index, hreflang, and sitemap.
...
- A complete event requires native-language title, summary, detailed body,
  agenda, programme, 25 SEO-oriented FAQ, metadata, schema, image labels, and
...
the rule can be enforced in code.
...
> **LEGACY AI WARNING:** the routes and files below document historical
> pipelines and may still mention Sonnet or Anthropic. Do not execute or restore
> any text-generation step in those routes, including `dryRun` calls that still
> rewrite before returning. The supported workflow is local generation in the
> active Codex session followed by validated submission through
> `publish-prepared`, as defined by `nightlife-milan-publishing` and
> `.agents/skills/import-events-local`.
...
Dati UFFICIALI dei venue (prezzi/orari/età/dress code reali, non scraping) via le pagine pubbliche xceed.me (HTTP puro). Piano: `.Codex/plans/2026-07-07-xceed-affiliate-pipeline.md`.
