...
import { GUE_JUST_ME_CANONICAL_SLUG } from './gueJustMe';
import { getGueJustMeLocalizedContent } from './gueJustMeLocales';
import { BAD_BUNNY_ARIA_CANONICAL_SLUG } from './badBunnyAria';
import { getBadBunnyAriaLocalizedContent } from './badBunnyAriaLocales';
import { MONDAY_NIGHT_CANONICAL_SLUG, getMondayNightLocalizedContent } from './weeklyJuly20Pilot';
import { getWeeklyJuly20LocalizedContent } from './weeklyJuly20Site';
...
export function getLocalizedEventContent(slug: string, locale: string): LocalizedEventContent | null {
  ...
  if (profile.canonicalSlug === GUE_JUST_ME_CANONICAL_SLUG) {
    return getGueJustMeLocalizedContent(pack.locale);
  }
  if (profile.canonicalSlug === BAD_BUNNY_ARIA_CANONICAL_SLUG) {
    return getBadBunnyAriaLocalizedContent(pack.locale);
  }
  if (profile.canonicalSlug === MONDAY_NIGHT_CANONICAL_SLUG) {
    return getMondayNightLocalizedContent(pack.locale);
  }
  const weeklyJuly20Content = getWeeklyJuly20LocalizedContent(profile, pack.locale);
  if (weeklyJuly20Content) return weeklyJuly20Content;
...
export function getLocalizedEventSeed(slug: string, locale: string): Event | null {
  ...
  const specializedContent = (targetLocale: LocaleCode) => {
    ...
    if (profile.canonicalSlug === GUE_JUST_ME_CANONICAL_SLUG) return getGueJustMeLocalizedContent(targetLocale);
    if (profile.canonicalSlug === BAD_BUNNY_ARIA_CANONICAL_SLUG) return getBadBunnyAriaLocalizedContent(targetLocale);
    if (profile.canonicalSlug === MONDAY_NIGHT_CANONICAL_SLUG) return getMondayNightLocalizedContent(targetLocale);
    const weeklyJuly20Content = getWeeklyJuly20LocalizedContent(profile, targetLocale);
    if (weeklyJuly20Content) return weeklyJuly20Content;
